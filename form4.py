...
def getvals():
    ...
    cursor.execute('INSERT INTO registrations (name, phone, email) VALUES (%s, %s, %s)', (name, phone, email))
...
def edit_record():
    ...
    if new_name and new_phone and new_email:
        # Update the selected record in the MySQL database
        cursor.execute('''
...
def delete_record():
    ...
    if confirmation:
        ...
        # Delete the selected record from the MySQL database
        cursor.execute('DELETE FROM registrations WHERE id=%s', (record_id,))
        # Commit the changes to the MySQL database
        conn.commit()
